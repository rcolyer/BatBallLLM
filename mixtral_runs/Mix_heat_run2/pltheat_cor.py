...
def StartFig(*args, **kwargs):
  ...
  plt.rcParams.update({'font.size': 13, 'text.usetex':True})
  return fig
...
df = pd.read_csv('output_corrected.csv', index_col=0)
fixedtemps = [float(s[1:]) for s in df['temp']]
df['temp'] = fixedtemps
StartFig()
...
plt.title('Accuracy vs LLM Temperature')
plt.xlabel('Temperature')
plt.ylabel('Accuracy')
plt.legend()
SaveFig('accuracy_temp_corr')
